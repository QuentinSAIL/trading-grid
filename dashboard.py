...
def build_price_chart(state: dict, width: int = 70, height: int = 15) -> Panel:
    """Build an ASCII price chart with buy/sell markers from filled orders."""
    ...
    # Collect price points from fills
    points = []
    for f in fills[-60:]:
        p = f.get("price", 0)
        side = f.get("side", "?")
        if p > 0:
            points.append((p, side))

    # Add current price as last point
    if price > 0:
        points.append((price, "current"))

    if len(points) < 2:
        txt = Text("\n  En attente de donnees...\n", style="dim italic")
        return Panel(txt, title="[bold] Prix [/bold]", border_style="bright_blue", height=height + 2)

    prices = [p for p, _ in points]
    # Include grid levels in min/max for context
    grid_prices = [o["price"] for o in grid_orders.values()]
    all_prices = prices + grid_prices if grid_prices else prices
    ...
    # Draw grid levels as dotted lines
    for gp in grid_prices:
        row = height - 1 - int((gp - p_min) / p_range * (height - 1))
    ...
    for i in range(n_points):
        ...
        p_val, side = points[i]
        row = height - 1 - int((p_val - p_min) / p_range * (height - 1))
        row = max(0, min(height - 1, row))

        if side == "buy":
            chart[row][col] = "▲"
            styles[row][col] = "bold green"
        elif side == "sell":
            chart[row][col] = "▼"
            styles[row][col] = "bold red"
        elif side == "current":
            chart[row][col] = "◆"
            styles[row][col] = "bold yellow"
        else:
            chart[row][col] = "●"
            styles[row][col] = "white"

        # Draw connecting line between consecutive points
        if i > 0:
            prev_col = int((i - 1) / max(n_points - 1, 1) * (width - 1))
            prev_col = max(0, min(width - 1, prev_col))
            prev_p, _ = points[i - 1]
            prev_row = height - 1 - int((prev_p - p_min) / p_range * (height - 1))
            ...
            for s in range(1, steps):
                ic = prev_col + int(dc * s / steps)
                ir = prev_row + int(dr * s / steps)
                if 0 <= ir < height and 0 <= ic < width and chart[ir][ic] == " ":
                    # Choose line character based on direction
                    if abs(dr) < abs(dc) * 0.3:
                        chart[ir][ic] = "─"
                    elif abs(dc) < abs(dr) * 0.3:
                        chart[ir][ic] = "│"
                    elif (dr > 0) == (dc > 0):
                        chart[ir][ic] = "╲"
                    else:
                        chart[ir][ic] = "╱"
                    styles[ir][ic] = "bright_blue"

    # Render to Text with price axis labels
    result = Text()
    label_w = 10
    for r in range(height):
        # Price label on the left
        if r == 0:
            label = f"{p_max:>{label_w}.2f} │"
        elif r == height - 1:
            label = f"{p_min:>{label_w}.2f} │"
        elif r == height // 2:
            mid = (p_max + p_min) / 2
            label = f"{mid:>{label_w}.2f} │"
        else:
            label = " " * label_w + " │"
        ...
        for c in range(width):
            ch = chart[r][c]
            st = styles[r][c]
            result.append(ch, style=st if st else "bright_black")
        result.append("\n")

    # Bottom axis
    result.append(" " * label_w + " └" + "─" * width, style="dim")
    ...
    result.append(" NOW  ", style="dim")
    result.append("·", style="green dim")
    result.append(" grid buy  ", style="dim")
    result.append("·", style="red dim")
    result.append(" grid sell", style="dim")

    return Panel(result, title="[bold] Historique des prix [/bold]", border_style="bright_blue")
...
def build_dashboard(state: dict) -> Layout:
    ...
    layout["right"].update(build_fills_table(state))
